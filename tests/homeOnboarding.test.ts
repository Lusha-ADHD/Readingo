...
test("chaque âge conserve la recommandation liée à la compétence", () => {
  for (const age of ["5", "6", "7", "other"] as const) {
    ...
    assert.equal(recommendHomeGame("knows-letters", age), "syllabes");
    assert.equal(recommendHomeGame("reads-syllables", age), "syllabes");
...
test("deux sauvegardes sans récence proposent les deux jeux", () => {
  const storage = createStorage({
    "readingo:lettres:v1": JSON.stringify({ sessions: 1 }),
    "readingo:syllabes:v3": JSON.stringify({ sessions: 2, completedWords: ["bateau"] }),
  });

  assert.deepEqual(readResumeState(storage).resumeGames, ["letters", "syllabes"]);
});
...
test("le dernier jeu valide devient l’action de reprise principale", () => {
  const storage = createStorage({
    "readingo:lettres:v1": JSON.stringify({ sessions: 1 }),
    "readingo:syllabes:v3": JSON.stringify({ sessions: 2, unlockedLevel: 2 }),
    [HOME_LAST_GAME_STORAGE_KEY]: JSON.stringify({ gameId: "syllabes", updatedAt: 42 }),
  });

  assert.deepEqual(readResumeState(storage).resumeGames, ["syllabes"]);
  assert.equal(readResumeState(storage).lastGame, "syllabes");
});
